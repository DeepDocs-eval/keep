...
If you have a connected provider and want to associate incoming alerts with it instead of creating a linked provider, append /{provider_id} to the webhook URL path after the provider type.
...
alertmanager:
  config:
    receivers:
      - name: "keep"
        webhook_configs:
          - url: "https://api.keephq.dev/alerts/event/prometheus/your_provider_id"
```
